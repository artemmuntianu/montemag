	...
	<div class="cart-contents">
		...
		<div class="cart-content-list">
			...
			<div class="cart-lower">
				...
				<a href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>" class="button left" title="Оплата">Оплата</a>
				<a href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" class="button right" title="Корзина">Корзина</a>
				
				<span class="subtotal">
					<?php
					echo 'Итого: <span>' . $cart_subtotal .'</span>';
					?>
				</span>
...
function shopera_woocommerce_breadcrumbs( $defaults ) {
	...
	$defaults['wrap_before'] = '<nav class="woocommerce-breadcrumb" itemprop="breadcrumb"><div class="woo-breadcrumb-content"><div class="woo-breadcrumb-inner">';
...
function custom_jquery_add_to_cart_script(){
    if (is_product()) {
        ?>
            <script type="text/javascript">
                (function($){
                    function onClickProductAddToCard(e) {
                        var productPrice = parseFloat(jQuery('.product .summary .price ins .woocommerce-Price-amount.amount').text());
                        fbq('track', 'AddToCart', { value: productPrice, currency: 'UAH' });
                    }
                    $('form.cart').on('submit', onClickProductAddToCard);
                })(jQuery);
            </script>
        <?php
    } 
    elseif (is_shop() || is_product_category()) {
        ?>
            <script type="text/javascript">
                (function($){
                    function onClickCategoryAddToCard(e) {
                        var productSku = e.target.attributes['data-product_id'].value;
                        var productPrice = parseFloat(jQuery('.products .post-'+productSku+' .price ins .woocommerce-Price-amount.amount').text());
                        fbq('track', 'AddToCart', { value: productPrice, currency: 'UAH' });
                    }
                    $('a.button.ajax_add_to_cart').on('click', onClickCategoryAddToCard);
                })(jQuery);
            </script>
        <?php
    }
